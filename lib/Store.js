...
class DatabaseConnection {
  ...
  async openStore(databaseName, storeName) {
    for(let i = 0; i < this.openStores.length; i++) {
      if(this.openStores[i].storeName == storeName && this.openStores[i].databaseName == databaseName) return i
    }
...
class Store {
  ...
  objectObservable(id) {
    ...
    observableValue.respawn = () => {
      observableValue.requestId = this.connection.observe(this.databaseName, this.storeName, id, onValue, onError)
      oldRespawn()
    }
    observableValue.requestId = this.connection.observe(this.databaseName, this.storeName, id, onValue, onError)
